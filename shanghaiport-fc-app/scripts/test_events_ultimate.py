#!/usr/bin/env python3
"""
上海海港2023赛季完整数据抓取器 - 终极版
提取原始HTML，在Python中解析events
"""
...
def scrape_match(url: str, match_info: dict) -> dict:
    ...
    try:
        ...
        # 2. 提取原始events数据（JavaScript只提取文本和links）
        extract_js = r"""(() => {
  const result = {events: []};
...
if __name__ == '__main__':
    ...
    if data:
        ...
        for i, e in enumerate(data['events'][:15], 1):
            print(f"{i}. {e['minute']}' - {e['type']} - {e['player']} ({e['team']})")
    else:
        print("\n❌ 失败")
