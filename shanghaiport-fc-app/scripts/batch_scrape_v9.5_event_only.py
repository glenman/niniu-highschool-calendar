#!/usr/bin/env python3
"""
上海海港赛事报告完整抓取器 v9.5 - 终极修复版
基于v9.3成功版本 + 点球识别优化
只从 .event 元素提取，不使用页面文本提取
"""
...
def scrape_match(url, match_info):
    try:
        ...
        for raw in extracted.get('events', []):
            ...
            team = 'home' if 'event a' not in className else 'away'
        ...
        match_data = {
            ...
            "metadata": {
                ...
                "version": "9.5-event-only-penalty-fix"
            }
        }
...
def main(season):
    print("="*70)
    print(f"上海海港{season}赛季完整数据抓取器 v9.5")
    print("基于v9.3成功版本 + 点球识别优化")
    print("="*70)
    print()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("用法: python3 batch_scrape_v9.5_event_only.py <赛季>")
        print("示例: python3 batch_scrape_v9.5_event_only.py 2023")
        sys.exit(1)
