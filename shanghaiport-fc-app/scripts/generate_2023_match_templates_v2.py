...
def create_match_template(date: str, round_num: int, match_id: str = url: str) -> dict:
    """创建单场比赛的数据框架"""
    ...
    return {
        ...
        "metadata": {
            ...
            "version": "1.0",
            "status": "template",
            "note": "⚠️ Cloudflare保护，无法自动抓取。需要手动填充数据。访问 https://fbref.com/en/matches/{match_id} 获取详细信息。可参考COMPLETE_SCRAPER_GUIDE.md了解需要填充的字段。"
        }
    }
...
def main():
    ...
    for i, match in enumerate(matches, 1):
        ...
        # 创建模板
        template = create_match_template(date, i, match_id, url)
