#!/usr/bin/env python3
"""
重新抓取2025赛季数据 - 使用正确的matchweek提取逻辑
"""
...
# 直接使用已验证的batch_scrape_2025.py的提取逻辑，但添加matchweek提取
EXTRACT_JS = r"""(() => {
  const result = {
    scores: [],
    formations: [],
  ...
  eventDivs.forEach(div => {
    ...
    if (links.length > 0) {
      result.events.push({text, className, players: links});
...
def main():
    print("="*70)
    print("上海海港2025赛季完整数据批量抓取器 v9.1")
