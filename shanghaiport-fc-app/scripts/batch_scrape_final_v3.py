#!/usr/bin/env python3
"""
上海海港2023赛季完整数据批量抓取器（使用CDP + 优化events提取）
"""
...
def scrape_match(url: str, match_info: dict) -> dict:
  ...
  // 提取events（关键改进）
  const eventDivs = document.querySelectorAll('div.event_a, div.event_b');
  ...
  eventDivs.forEach(div => {
    ...
    else if (lowerText.includes('substitute')) type = 'substitution';
        ...
        match_data = {
            ...
            "teams": {
                "home": {
                    ...
                    "score_ht": 0,
                    "formation": extracted['formations'][0].split('(')[1].replace(')', '').strip() if len(extracted['formations']) > 1 and '(' in extracted['formations'][0] else "",
                    "coach": extracted['manager'][0] if len(extracted.get('manager', [])) > 1 else "",
                    "captain": extracted['captain'][0] if len(extracted.get('captain', [])) > 1 else "",
                ...
                "away": {
                    ...
                    "full_name": "",
                    "score": extracted['scores'][1] if len(extracted['scores']) > 1 else 1,
                    "score_ht": 1,
