...
学习自 complete_scraper_with_events.py 的events提取逻辑
"""
...
def scrape_match(url: str, match_info: dict) -> dict:
    ...
    try:
        ...
        # 2. 提取完整数据（学习自complete_scraper_with_events.py）
        extract_js = """
        (() => {
          const result = {
            scores: [],
          ...
          // ===== 关键：Events提取（学习自complete_scraper_with_events.py）=====
          // 遍历所有div元素查找包含时间标记的
          document.querySelectorAll('div').forEach(div => {
            ...
            if (timeText) {
              ...
              if (links.length > 0) {
                ...
                if (text.includes('Own Goal')) {
                ...
                } else if (text.toLowerCase().includes('for') && links.length >= 2) {
                  ...
                  eventType = 'red_card';
                  player = links[0]?.textContent?.trim() || '';
                } else if (links.length >= 1 && !text.toLowerCase().includes('for')) {
                  // 可能是进球
                  eventType = 'goal';
                  player = links[0]?.textContent?.trim() || '';
                }

                // 判断主客队（通过div的class或位置）
                if (div.className && div.className.includes('event a')) {
                  team = 'home';
                } else if (div.className && div.className.includes('event b')) {
                  team = 'away';
                }

                if (player && eventType !== 'unknown') {
                  result.events.push({
                    minute: parseInt(timeText[1]),
                    type: eventType,
                    player: player,
                    assist: assist,
          ...
          // 教练
          const managerMatch = document.body.innerHTML.match(/Manager:\\s*([^<\\n]+)/g);
          if (managerMatch) {
