#!/usr/bin/env python3
"""
上海海港赛事报告抓取器 v9.3 Final
通用版：支持2023/2024/2025所有赛季
"""
...
EXTRACT_JS = r"""(() => {
  ...
  eventDivs.forEach(div => {
    ...
    if (links.length > 0) {
      ...
      if (text.includes('Red') && text.includes('Card')) {
...
def main(season):
    print("="*70)
    print(f"上海海港{season}赛季完整数据抓取器 v9.3 Final")
    print("="*70)
    print()
    ...
    # 判断数据格式（2023/2024是直接数组，2025有match_urls字段）
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("用法: python3 batch_scrape_all_seasons_v9.3.py <赛季>")
        print("示例: python3 batch_scrape_all_seasons_v9.3.py 2023")
        sys.exit(1)
