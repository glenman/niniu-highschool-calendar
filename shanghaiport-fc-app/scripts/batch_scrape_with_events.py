#!/usr/bin/env python3
"""
上海海港2023赛季完整数据批量抓取器（含Events）
使用CDP提取FBref全部30场比赛数据，包括比赛事件
"""
...
def scrape_match(url: str, match_info: dict) -> dict:
  ...
  allDivs.forEach(div => {
    ...
    if (timeMatch && text.length < 500) {
      ...
      if (lowerText.includes('goal') && !lowerText.includes('own goal')) type = 'goal';
      else if (lowerText.includes('own goal')) type = 'own_goal';
      else if (lowerText.includes('yellow card')) type = 'yellow_card';
      else if (lowerText.includes('red card')) type = 'red_card';
      else if (lowerText.includes('substitute') || lowerText.includes('for ')) type = 'substitution';
      
      // 只保存有意义的事件（忽略导航菜单等）
      if (type !== 'unknown' && text.includes('Wuhan Three Towns') || text.includes('Shanghai Port')) {
        // 提取球员名字（从链接中）
        ...
        if (divClass.includes('event') && divClass.includes('b')) {
          team = 'away';
        } else if (players.length > 0) {
          // 根据球员判断
          const homePlayers = data.players.home.starters.map(p => p.name);
          if (players.some(p => !homePlayers.includes(p))) {
            team = 'away';
          }
        }
  ...
  data.events = uniqueEvents.sort((a, b) => a.minute - b.minute);
  ...
  data.stats.yellow_cards = {home: yellowHome, away: yellowAway};
  data.stats.red_cards = {home: redHome, away: redAway};
...
def main():
    print("="*70)
    print("上海海港2023赛季完整数据批量抓取（含Events）")
    print("="*70)
    print()
    ...
    for i, match in enumerate(matches, 1):
        ...
        if match_data:
            ...
            print(f"         球员: {hl}+{al} | 事件: {events_count}个 | 统计: {stats_count}项")
            success_count += 1
        else:
            print(f"      ❌ 失败")
