...
EXTRACT_JS = r"""(() => {
  ...
  // 3. 提取教练和队长（需要区分主客队）
  const sections = bodyText.split(/Shanghai Port|Shenzhen Peng City/);
  
  // 主队（Shanghai Port）
  const homeIndex = bodyText.indexOf('Shanghai Port');
  if (homeIndex !== -1) {
    const homeSection = bodyText.substring(homeIndex, homeIndex + 800);
    const homeManagerMatch = homeSection.match(/Manager:\s*([^\n]+)/);
    const homeCaptainMatch = homeSection.match(/Captain:\s*([^\n]+)/);
    if (homeManagerMatch) result.homeManager = homeManagerMatch[1].trim();
    if (homeCaptainMatch) result.homeCaptain = homeCaptainMatch[1].trim();
  }

  // 客队（需要从对阵信息中提取）
  const awayIndex = bodyText.indexOf('\n1\n0-0-1'); // 找到客队部分
  if (awayIndex !== -1) {
    const awaySection = bodyText.substring(awayIndex, awayIndex + 800);
    const awayManagerMatch = awaySection.match(/Manager:\s*([^\n]+)/);
    const awayCaptainMatch = awaySection.match(/Captain:\s*([^\n]+)/);
    if (awayManagerMatch) result.awayManager = awayManagerMatch[1].trim();
    if (awayCaptainMatch) result.awayCaptain = awayCaptainMatch[1].trim();
  }
  ...
  if (officialsMatch) {
    ...
    const mainRefMatch = officialsText.match(/([^(·]+?)\s*\(Referee\)/);
    if (mainRefMatch) result.referees.main = mainRefMatch[1].trim();
    
    const ar1Match = officialsText.match(/·\s*([^(·]+?)\s*\(AR1\)/);
    if (ar1Match) result.referees.ar1 = ar1Match[1].trim();
    
    const ar2Match = officialsText.match(/·\s*([^(·]+?)\s*\(AR2\)/);
    if (ar2Match) result.referees.ar2 = ar2Match[1].trim();
    
    const fourthMatch = officialsText.match(/·\s*([^(·]+?)\s*\(4th\)/);
    if (fourthMatch) result.referees.fourth = fourthMatch[1].trim();
    
    const varMatch = officialsText.match(/·\s*([^(·]+?)\s*\(VAR\)/);
    ...
    try:
        ...
        match_data = {
            "match_info": {
                ...
                "time": extracted.get('matchTime', '20:00'),  # 使用提取的比赛时间
                "competition": {
            ...
            "teams": {
                "home": {
                    ...
                    "coach": extracted.get('homeManager', ''),  # 教练
                    "captain": extracted.get('homeCaptain', ''),  # 队长
                ...
                "away": {
                    ...
                    "coach": extracted.get('awayManager', ''),  # 教练
                    "captain": extracted.get('awayCaptain', ''),  # 队长
...
def main():
    print("="*70)
    print("上海海港2025赛季完整数据批量抓取器 v9.2")
    print("包含：比赛时间、教练、队长、裁判组、进球类型")
    print("="*70)
    print()
    ...
    for i, match in enumerate(matches, 1):
        ...
        if match_data:
            ...
            main_ref = match_data['match_info']['referee']['main']
            
            print(f"      ✅ {home['name']} {hs}-{as_} {away['name']}")
            print(f"         时间:{time_info} | 教练:{home_coach} | 裁判:{main_ref}")
